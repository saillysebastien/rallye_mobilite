...
if (isset($_POST['upload_rebus'])) {
  ...
  $update = sprintf("UPDATE appli SET title='gpe3pg4', name='$name', one='$one', two='$two', three='$three', four='$four', five='$five', indice='$index', response='$response', question ='$question', my_all='$my_all', image='$image' WHERE id='24'");
...
if (isset($_POST['locate'])) {
  ...
  if ($valid) {
    $update = sprintf("UPDATE locate SET name='gpe3pg4', start_traject='$start', end_traject='$end', qr_code='afpa-70x70.png' WHERE id='24'");
    $update_locate = mysqli_query($db, $update);
    $sql_qr = "SELECT * FROM locate WHERE id = '24'";
