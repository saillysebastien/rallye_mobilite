...
if (isset($_GET['id']) && !empty(trim($_GET['id'])) && $_GET['id'] != 0) {
  ...
  $valid = false;
  $errors['id'] = 'Vous devez spécifier une entreprise à supprimer';
}
...
if ($valid) {
  ...
  try {
    ...
    if ($delete) {
      ...
      $informations['delete'] = "<div class='alert alert-danger center'>  L'image $image supprimée du dossier et l'entreprise $title supprimée de la base de données</div>\n
      <br/>
      <a class='btn btn-primary' href='enterprises.php'>Retour à la liste des entreprises</a>";
    }
  } catch (Exception $e) {
